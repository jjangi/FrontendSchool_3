...
- about:blank (다른 브라우저의 내부 변수를 막기 위함)
...
압축 -> JSCompress
난독화 -> JavaScript Obfuscator Tool 4. 주석
...
- var, let, const
  ...
  - let(ES5) : 블록 레벨 스코프, 재선언시 애러 O, 콘솔에서는 에러 X, 변경가능한 자료형
  - const(ES5) : 블록 레벨 스코프, 재선언시 애러 O, 콘솔에서는 에러 X, 변경이 불가능한 자료형(상수)
...
- nullish 병합 연산자(??)
  ...
  - ?? 앞의 값이 null이나 undefined면 뒤의 값을, 아니면 앞의 값을.
...
- String(문자열)
  ...
  - 메서드 :
    ...
    - str.concat()
    - str.repeat()
    - str.splice() -> (Update 22.10.13) X, 배열에 있는 메소드입니다. str에는 없습니다.
    ```js
    "hello".repeat(100);
    "0".repeat(100);
...
- switch
  ...
  if (true) {
    console.log("hello 3"); //출력
  }
  if (true) {
    console.log("hello 4"); //출력
  }
  ```
  ```js
  if (false) {
...
- 함수 표현식과 함수 선언식
  ```js
  let 함수표현식 = function () {}; // 호이스팅 X(tdz에 들어가서 안되는 것처럼 보인다.)
...
- prompt('hello') //prompt로 얻어진 값은 모두 string.
- comfirm('hello')
- alert('hello')
